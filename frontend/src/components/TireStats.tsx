    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <h3 className="mt-2 font-display text-2xl uppercase tracking-tight">
            {leaderStats?.name ?? 'Awaiting telemetry'}
          </h3>
          <p className="mt-1 font-body text-sm text-[var(--color-muted-fg)]">{leaderStats?.team ?? 'No leader yet'}</p>
        </div>
        <div
      ...
      <div className="mt-4 grid grid-cols-2 gap-3">
        <div className="border-2 border-[var(--color-border)] bg-[var(--color-muted)] p-3 text-center">
          ...
          <p className="mt-2 font-display text-3xl leading-none">{leaderStats?.tyreAge ?? 0}</p>
